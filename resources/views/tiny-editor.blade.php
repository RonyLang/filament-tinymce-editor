...
>
    <div
        ...
        x-init="(() => {
            // 可见时再初始化；若不可见，使用 IntersectionObserver 监听显示
            const isVisible = () => {
                const el = $el; // wrapper div
                return !!(el && el.offsetParent !== null && el.offsetWidth > 0 && el.offsetHeight > 0);
            };

            const tryInit = () => { if (!initialized) { (window.requestIdleCallback || ((cb)=>setTimeout(cb, 0)))(() => initTiny()); } };
